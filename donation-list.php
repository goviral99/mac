...
/*$i = $stripe->subscriptions->search([
	'limit' => 40,
	'query' => 'status:\'active\' or status:\'canceled\'',
	// 'query' => 'status:\'active\'',
]);
echo "<pre>"; print_r($i); die("</pre>");*/

// $i = $stripe->invoices->retrieve(
// 	'in_1LT2VeHXmZTJVQtNUEHBdFYn',
// 	[]
// );
/*foreach ($i as $key => $value) {
	echo "<pre>"; print_r($value); die("</pre>");
}
echo "<pre>"; print_r($i); die("</pre>");*/
...
if (isset($_POST['trx_rf']) && !empty($_POST['trx_rf'])) {
    ...
    $headers .= 'From: "Hands for Charity Refund" noreply@handsforcharity.org';

    mail($to, $subject, $message, $headers);
	
	$query = "SELECT * FROM `transactions` WHERE `uid`='{$user_id}' and `trx_id`='{$trx_id}' limit 1";
	...
	while ( $data = mysqli_fetch_array($exicution) ) {
		// $refund = $stripe->refunds->create(['payment_intent' => 'pi_3LSMPcHXmZTJVQtN1SRajsj7']);
		// $stripe->paymentIntents->cancel('pi_32AkjQ5H4Bas2eAolX13', []);
		$refund = $stripe->refunds->create(
			['payment_intent' => $data['trx_id']]
		);		
		if (!empty($refund)) {
			if ($refund['object'] == 'refund' && $refund['status'] == 'succeeded') {
				$refund_id = $refund['id'];
				$r_object = $refund['object'];
				$update_query = "UPDATE `transactions` SET `refund_id`='{$refund_id}', `refund` = '{$r_object}' WHERE `trx_id`='{$trx_id}';";
...
if (isset($_POST['trx_c']) && !empty($_POST['trx_c'])) {
	$sub_id = $_POST['trx_c'];
	$query = "SELECT * FROM `transactions` WHERE `uid`='{$user_id}' and `sub_id`='{$sub_id}' limit 1";
	$exicution = mysqli_query($con, $query) or die(mysqli_error($con) ."<br>". $query);
	if (mysqli_num_rows($exicution) < 1) { die( "No Records Found." ); }	
	while ( $data = mysqli_fetch_array($exicution) ) {
		...
		if (!empty($c)) {
			if ($c['object'] == 'subscription' && $c['status'] == 'canceled') {
				$c_id = $c['id'];
				$c_canceled_at = $c['canceled_at'];
				$update_query = "UPDATE `transactions` SET `cancel_id`='{$c_canceled_at}', `canceled` = 'canceled' WHERE `sub_id`='{$sub_id}';";
				$result = mysqli_query($con,$update_query);
				if ($result) {
					echo mysqli_affected_rows($con) . "<br>";
				}else{
...
<body>
	<div class="dashboard_wrapper position-relative d-flex overflow-hidden">
		<div class="dashboard_sidebar d-flex flex-column">
			<a href="<?php echo website_url ?>" class="logo">
			...
			<ul class="list-unstyled mb-0 side_nav mt-4 mb-4">
				...
				<li>
					<a href="<?php echo website_url ?>donation-list.php" class="active">
		...
		<div class="dashboard_content w-100">
			...
			<div class="dashboard_content_table mt-md-5 mt-4 p-4">
				...
				<div class="table-responsive position-relative">
					<div class="table_loader d-flex align-items-center justify-content-center">
					...
					<table class="table table-hover mb-0">
						...
						<tbody>
							<?php 
							$query = "SELECT * FROM `transactions` WHERE `uid`='{$_SESSION['user_id']}' and `type` IN ('monthly','yearly') ORDER BY `timestamp` desc";
							$exicution = mysqli_query($con, $query) or die(mysqli_error($con) ."<br>". $query);
											...
											<ul class="dropdown-menu dropdown-menu-end action_btns" aria-labelledby="dropdownMenuButton1">
												<li>
													<?php if ($data['refund'] != 'refund' ){ ?>
														<!-- <a data-rf='<?php echo $data['trx_id'] ?>' class="dropdown-item">
															Refund
														</a> -->
												...
												-->
													<?php if ($data['canceled'] != 'canceled') { ?>
														<a data-cause='<?php echo $data['cause'] ?>' data-c='<?php echo $data['sub_id'] ?>' class="dropdown-item" data-bs-toggle="modal" data-bs-target="#cancelSubs">
															Cancel
														</a>
													<?php } else { ?>
														<a class="dropdown-item">
	...
	<div class="modal fade" id="cancelSubs" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="cancelSubsLabel" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="cancelSubsLabel">ThankYou</h5>
				</div>
				<div class="modal-body">
