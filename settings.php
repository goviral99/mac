...
if (isset($_POST) && !empty($_POST['uf_name'])) {
	$update = false;
	$update_transactions = false;
	$_SESSION['security_error'] = '';
	$_SESSION['security_update'] = '';
	...
	if (isset($_POST['change_pass']) && $_POST['change_pass'] == 'change_password') {
		if ($userPass != $_POST['password']) {
			$_SESSION['security_error'] = 'Old Password is wrong please enter correct password';
		} elseif ($_POST['n_password'] != $_POST['cn_password']) {
			$_SESSION['security_error'] = 'New Passsword and Confirm New Password do not match';
		} else {
			$userPass = $_POST['n_password'];
			$_SESSION['security_update'] .=	 '<b>Password</b> ';
			$update = true;
		}
	}
	if ($_SESSION['security_error'] == '') {
		if (!empty($_POST['uf_name']) && ($f_name != $_POST['uf_name'])) {
			$f_name = $_POST['uf_name'];
			$update = true;
			$update_transactions = true;
			$_SESSION['security_update'] .=	 '<b>First Name</b> ';
		}
		if (!empty($_POST['ul_name']) && ($l_name != $_POST['ul_name'])) {
			$l_name = $_POST['ul_name'];
			$update = true;
			$_SESSION['security_update'] .=	 '<b>Last Name</b> ';
		}
		if ($update == true) {
			$_SESSION['security_update'] .=	 'Updated';
			$update_query = "UPDATE `users` SET `first_name`='{$f_name}', `last_name`='{$l_name}', `password`='{$userPass}', `token`=''  WHERE `id`='{$user_id}'";
			$result = mysqli_query($con,$update_query);
			if ($result) {
				$_SESSION['user_first_name'] = $f_name;
				$_SESSION['user_last_name'] = $l_name;
			}
			if ($update_transactions == true) {
				$update_query = "UPDATE `transactions` SET `name`='{$f_name}'  WHERE `uid`='{$user_id}';";
				$result = mysqli_query($con,$update_query);
			}
		}
		$_SESSION['security_error'] .= $_SESSION['security_update']; 
	}
}
?>
<!DOCTYPE html>
	...
	<div class="dashboard_wrapper position-relative d-flex overflow-hidden">
		<div class="dashboard_sidebar d-flex flex-column">
			...
			<ul class="list-unstyled mb-0 side_nav mt-4 mb-4">
				<li>
					<a href="<?php echo website_url ?>dashboard.php">
		...
		<div class="dashboard_content w-100">
			...
			<?php if (isset($_SESSION['security_error']) && !empty($_SESSION['security_error'])): ?>
			<div class="alert alert-warning alert-dismissible fade show mt-md-5 mt-4" role="alert">
			...
			<?php echo $_SESSION['security_error'];  $_SESSION['security_error'] = ''?>
			<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
		</div>
			<?php endif ?>
			<form method="post" action="" class="row login_form_wrap setting_form_wrap mt-md-5 mt-4">
				<div class="col-md-6 mb-4 mb-md-0">
					<div class="dashboard_content_table h-100 py-4 px-md-3 px-3">
						<h5 class="mb-3"><b>Personal Information</b></h5>
						<div class="position-relative my-3">
							<label><b>First Name</b></label>
							<input type="text" name="uf_name" class="form-control" value="<?php echo $_SESSION['user_first_name'] ?>">
						</div>
						<div class="position-relative my-3">
							<label><b>Last Name</b></label>
							<input type="text" name="ul_name" class="form-control" value="<?php echo $_SESSION['user_last_name'] ?>">
						</div>
						<div class="position-relative">
							<label><b>Email</b></label>
							<input type="email" name="user_email" class="form-control" value="<?php echo $_SESSION['user_email'] ?>" disabled>
						</div>
					</div>
				</div>
				<div class="col-md-6 mb-4 mb-md-0">
					<div class="dashboard_content_table h-100 py-4 px-md-3 px-3">
						<h5 class="mb-3"><b>Security Information</b></h5>
						<div class="position-relative my-3">
							<label><b>Old Password</b></label>
							<div class="position-relative">
								<input type="password" name="password" class="form-control pe-4">
						...
						<input type="checkbox" id="new_password" class="d-none" name="change_pass" value="change_password">
						<label class="forgot_link text-end d-block" for="new_password">Change Password</label>
						<div class="change_poassword_fields">
							<div class="position-relative my-3">
								<label><b>New Password</b></label>
								<div class="position-relative">
									<input type="password" name="n_password" class="form-control pe-4">
									<button type="button" class="password_toggle_btn">
										<span class="position-relative">
											<img src="includes/images/eye.svg" class="img-fluid" alt="">
										</span>
							...
							<div class="position-relative">
								...
								<div class="position-relative">
									<input type="password" name="cn_password" class="form-control pe-4">
									<button type="button" class="password_toggle_btn">
										<span class="position-relative">
											<img src="includes/images/eye.svg" class="img-fluid" alt="">
										</span>
				...
				<div class="col-md-12">
					<button type="submit" class="mt-4 w-auto px-5">Save</button>
				</div>
			</form>
		</div>
	</div>
